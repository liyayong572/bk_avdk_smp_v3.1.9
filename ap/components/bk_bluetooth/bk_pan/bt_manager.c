...
void gap_event_cb(bk_gap_bt_cb_event_t event, bk_bt_gap_cb_param_t *param)
{
    switch (event)
    {
        ...
        case BK_BT_GAP_ACL_CONN_CMPL_STAT_EVT:
        {
            ...
            {
                LOGD("Connected to %02x:%02x:%02x:%02x:%02x:%02x\n", addr[5], addr[4], addr[3], addr[2], addr[1], addr[0]);
        ...
        case BK_BT_GAP_AUTH_CMPL_EVT:
        {
            ...
            {
                LOGD("(%02x:%02x:%02x:%02x:%02x:%02x)authentication success\n", addr[5], addr[4], addr[3], addr[2], addr[1], addr[0]);
            }
            else
            {
                ...
                LOGD("(%02x:%02x:%02x:%02x:%02x:%02x)authentication failed, status: 0x%02x\n", addr[5], addr[4], addr[3], addr[2], addr[1], addr[0], param->auth_cmpl.stat);
            }
        }
        break;
        ...
        {
            LOGD("%s recv linkkey %02X:%02X:%02X:%02X:%02X:%02X\n", __func__,
        ...
        case BK_BT_GAP_LINK_KEY_REQ_EVT:
        {
            ...
            if (ret >= 0)
            {
                LOGD("%s found link key %02X:%02X:%02X:%02X:%02X:%02X\n", __func__,
                     addr[5],
                     addr[4],
                     addr[3],
                     addr[2],
            ...
            {
                LOGD("%s not found link key in storage %02X:%02X:%02X:%02X:%02X:%02X\n", __func__,
                     addr[5],
                     addr[4],
                     addr[3],
                     addr[2],
