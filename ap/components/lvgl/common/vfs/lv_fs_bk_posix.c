...
#elif CONFIG_LVGL_V9
static lv_fs_res_t fs_dir_read_v9(lv_fs_drv_t * drv, void * dir_p, char * fn, uint32_t fn_len);
#endif
...
#elif CONFIG_LVGL_V9
    fs_drv.dir_read_cb = fs_dir_read_v9;
#endif
...
static lv_fs_res_t fs_dir_read_v9(lv_fs_drv_t * drv, void * dir_p, char * fn, uint32_t fn_len)
{
    ...
    do {
        entry = readdir(dir_p);
        if(entry) {
            if(entry->d_type == DT_DIR) os_snprintf(fn, fn_len, "/%s", entry->d_name);
            else os_strncpy(fn, entry->d_name, fn_len);
        }
        else {
            os_strncpy(fn, "", 1);
        }
