...
lv_obj_t * _lv_demo_music_list_create(lv_obj_t * parent)
{
    ...
    /* Get total track count and create buttons for each track */
    uint32_t total_tracks = _lv_demo_music_get_track_count();
    uint32_t track_id;
    for(track_id = 0; track_id < total_tracks; track_id++) {
    ...
    /* Only check button if there are songs in the list */
    if (total_tracks > 0) {
...
{
    /* Check if list has children before accessing */
...
static lv_obj_t * add_list_btn(lv_obj_t * parent, uint32_t track_id)
{
    ...
    /* All tracks are enabled since they're loaded from SD card */
    /* No need to disable tracks anymore */
