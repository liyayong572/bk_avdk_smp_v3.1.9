...
bk_err_t rlk_demo_dispatch(bk_rlk_recv_info_t *rx_info)
{
    ...
    else if((app_ptr->type == RLK_APP_TYPE_DATA)&&(app_ptr->subtype == RLK_APP_IPERF_CONNECTION_REQ))
    {
        rlk_iperf_connection_req_handler(rx_info);
    }
    else if((app_ptr->type == RLK_APP_TYPE_DATA)&&(app_ptr->subtype == RLK_APP_IPERF_DISCONNECTION))
    {
        rlk_iperf_disconnection_handler(rx_info);
    }
    else if((app_ptr->type == RLK_APP_TYPE_DATA)&&(app_ptr->subtype == RLK_APP_IPERF_DATA))
    {
        if (rlk_iperf_env.state == RLK_IPERF_STATE_STARTED)
        {
            ...
            os_memcpy(buf,data,app_ptr->len);
            ret = rtos_push_to_queue(&rlk_iperf_env.conn.recvmbox, &buf, BEKEN_NO_WAIT);
            if (ret != BK_OK)
            {
                os_free(buf);
                return BK_FAIL;
