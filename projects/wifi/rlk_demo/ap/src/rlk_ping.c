...
bk_err_t rlk_ping_response(bk_rlk_recv_info_t *rx_info)
{
    ...
    if (app_ptr == NULL)
    {
        RLK_LOGD("rlk_ping_response malloc failed\n");
        return BK_FAIL;
    }
    ...
    if (ret < 0)
    {
        RLK_LOGD("rlk_ping_send failed ret:%d\n", ret);
...
{
    BK_LOGD(NULL, "Usage: rlk_ping target [options]\n");
    BK_LOGD(NULL, "\n");
    BK_LOGD(NULL, "target:  must be mac address(full 6 byte or last 3 byte)\n");
    BK_LOGD(NULL, "option:\n");
    BK_LOGD(NULL, "       -c        ping count, default is 4 times\n");
    BK_LOGD(NULL, "       -i        interval, default is 1 second\n");
    BK_LOGD(NULL, "       -s        packetsize, default is 32byte\n");
    BK_LOGD(NULL, "       -t        timeout, default is 1 second\n");
    BK_LOGD(NULL, "       --stop    stop ping program\n");
    BK_LOGD(NULL, "       -h        help info\n");
...
void rlk_ping_stop(void)
{
    if (rlk_ping_env.state == RLK_PING_STATE_STARTED) {
        ...
        RLK_LOGD("rlk_ping: rlk_ping is stopping...\n");
    }
}
...
uint8_t rlk_ping()
{
    ...
    while (rlk_ping_env.state == RLK_PING_STATE_STARTED)
    {
        ...
        if (rlk_ping_send() >= BK_OK)
        {
            ...
            {
                recv_cnt++;
                RLK_LOGD("%d bytes from %s rlk_ping_seq=%d time=%d ticks\n", recv_len, rlk_ping_env.mac_str, send_times,
                        rlk_sys_now() - recv_start_tick);
            }
            else
            {
                RLK_LOGD("From %s rlk_ping_seq=%d timeout\n", rlk_ping_env.mac_str, send_times);
            }
        }
        else
        {
                RLK_LOGD("Send %s - error\n", rlk_ping_env.mac_str);
        }

        send_times++;
        if (send_times >= ping_counts)
        {
            RLK_LOGD("ping end, sent cnt: %d, recv cnt: %d, drop cnt: %d(%.1f%%)\n", 
                sent_cnt,
                recv_cnt,
...
void rlk_ping_start()
{
    if (rlk_ping_env.state == RLK_PING_STATE_STOPPED) 
    {
        if (rlk_ping_env.mac_addr) {
            ...
            rtos_smp_create_thread(NULL, RLK_PING_PRIORITY, "RLK ping",
            ...
            RLK_LOGD("Please input: rlk_ping <mac address>\n");

    }
    else if (rlk_ping_env.state == RLK_PING_STATE_STOPPING)
        RLK_LOGD("rlk_ping: rlk_ping is stopping, try again later!\n");
    else
        RLK_LOGD("rlk_ping: rlk_ping is running, stop first!\n");
}
